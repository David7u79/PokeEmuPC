...
public:
    explicit MelonDsEngine(const std::string& coreLibraryPath = "", std::shared_ptr<Pocket::Save::SaveSessionCoordinator> coordinator = std::make_shared<Pocket::Save::SaveSessionCoordinator>());
    explicit MelonDsEngine(std::shared_ptr<Pocket::Save::SaveSessionCoordinator> coordinator) : MelonDsEngine("", std::move(coordinator)) {}
    ~MelonDsEngine() override;
    bool loadRom(const std::string& romPath) override;
    void stop() override;
    const uint8_t* topFramebuffer() const; const uint8_t* bottomFramebuffer() const;
    size_t screenFramebufferSize() const;
    void sendTouchInput(int x, int y, bool isPressed); void getTouchInput(int& x, int& y, bool& isPressed) const;
    std::string loadedRomPath() const { return m_romPath; } std::string saveFilePath() const { return m_savePath; }
...
private:
    ...
    int m_touchX{0}, m_touchY{0}; bool m_touchPressed{false};
};
} // namespace Pocket::Emulator
